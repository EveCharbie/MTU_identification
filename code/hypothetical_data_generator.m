...
for i = 1 : size(a_num,1) % activation muscle
    for ii = 1 : length(qknee)
        for iii = 1 :  length(qankle)
            q_num = [0, 0 , 0, 0, (qknee(ii)/180)*pi , (qankle(iii)/180)*pi ] ; %
            ...
            rootedvariables_num = [full(fiberLength_num) ,full(tendonLengthening_num)] ; 
            ...
            % output
            ntrials = ntrials +1 ;
            Data(ntrials,:) = [Torque, (qknee(ii)/180)*pi , (qankle(iii)/180)*pi,  a_num(i,:),...
                fLength_num, phi_num ] ; % variables mesured 
