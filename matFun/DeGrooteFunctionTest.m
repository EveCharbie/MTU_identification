function [casadiFun,unknown_parameters,definition] = DeGrooteFunctionTest()
% import Casadi 
[~, name] = system('hostname');
if strcmp(name(1:8), '151302-1')
    addpath('C:\Users\amariani\Desktop\Thèse\Experimental et Revues\Manip_Neuromusculoskeletal_Modeling\Casadi')
...
% "q" is the spatial skeleton configuration (x, y, z, theta_hip,
% theta_knee, theta_ankle) [6]
% "knownparameter" is subject segment length and muscle insertion position.
% [33]
% segment_geometry, muscle_insertion, Local_ViaPoint_tibialis_anterior
...
muscleTendonParametersSingle =  vertcat(optimalFiberLength(1), phi0(1), maximalIsometricForce(1), tendonSlackLength(1)) ; % stock muscleTendonParameters
...
% tendonLengthing = (tendonLength - tendonSlackLength) ./ tendonSlackLength; 
...
normalizedTendonForceSingle = normalizedTendonForce(1);

tendonForce= normalizedTendonForce .* maximalIsometricForce ; % Non-normalized equation
tendonForceSingle= tendonForce(1);
...
normalizedMuscleActiveForceLengthSingle = normalizedMuscleActiveForceLength(1);

MuscleActiveForceLength = a .* normalizedMuscleActiveForceLength .* maximalIsometricForce ; % Non-normalized equation
MuscleActiveForceLengthSingle = MuscleActiveForceLength(1);


% Passive muscle force-length (S3)
kpe = 4.0 ; e0 = 0.6 ; % muscle parameters
...
normalizedMusclePassiveForceSingle = normalizedMusclePassiveForce(1);


musclePassiveForce = normalizedMusclePassiveForce .* maximalIsometricForce ; % Non-normalized equation
musclePassiveForceSingle = musclePassiveForce(1);
...
getMuscleActiveForce = Function('getMuscleActiveForceSingle', ...
    {all_states, muscleTendonParameters}, {MuscleActiveForceLength}, ...
    {'all_states','muscle_tendon_parameters'}, {'MuscleActiveForce'}) ;

getTendonForceSingle = Function('getTendonForceSingle', ...
    {tendonLength(1), muscleTendonParametersSingle}, {tendonForceSingle}, ...
    {'tendon_length','muscle_tendon_parameters'}, {'tendon_force_single_muscle'}) ;

getMuscleForceSingle = Function('getMuscleForceSingle', ...
    {a(1),fiberLength(1), muscleTendonParametersSingle}, {muscleForceSingle}, ...
    {'muscle_activation','fiber_length','muscle_tendon_parameters'}, {'muscle_force_single_muscle'}) ;

% normalizeTendonForceSingle = Function('normalizeTendonForceSingle', ...
%     {tendonLength(1), muscleTendonParametersSingle}, {normalizedTendonForceSingle}, ...
%     {'tendon_length','muscle_tendon_parameters'}, {'NormalizedTendonForce'});

getMusclePassiveForceSingle = Function('getMusclePassiveForceSingle', ...
    {fiberLength(1), muscleTendonParametersSingle}, {musclePassiveForceSingle}, ...
    {'fiber_length','muscle_tendon_parameters'}, {'muscle_passive_force_single_muscle'}) ;

getMuscleActiveForceSingle = Function('getMuscleActiveForceSingle', ...
    {a(1),fiberLength(1), muscleTendonParametersSingle}, {MuscleActiveForceLengthSingle}, ...
    {'muscle_activation','fiber_length','muscle_tendon_parameters'}, {'muscle_pactive_force_single_muscle'}) ;
...
known = vertcat(a(1), LUMT(1), muscleTendonParametersSingle) ; 
opts_kinsolSM = struct("constraints", ones(3, 1), ...    
    'abstol', 1e-15, ...
    'u_scale', 1./[0.1, 0.001, 0.1]',...
    'error_on_fail', false,...
    'max_iter',0,...
...
casadiFun = struct(...
    ...
    'getMomentArm', getMomentArm,...                                       % about musculoskeletal model 
    ...
    'normalizeTendonForce', normalizeTendonForce,...                       % about forces
    'getMusclePassiveForceSingle', getMusclePassiveForceSingle, ...
    'getMuscleActiveForceSingle', getMuscleActiveForceSingle, ...
    'getMuscleForceSingle', getMuscleForceSingle, ...
    'getTendonForceSingle', getTendonForceSingle,...                       % for force length representation 
    ...
    'equilibrateMuscleTendonSingleMuscle', equilibrateMuscleTendonSingleMuscle, ... % about equilibrium
