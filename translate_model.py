...
def translate_model(q: np.ndarray, biomod_filepath: str) -> None:
    ...
    merge_tool.add(MuscleMerge(name="gas_r", muscle_names=["med_gas_r", "lat_gas_r"]))
...
# Create the model with the knee straight
q_0 = np.zeros((nb_q,))
biomod_filepath_0 = f"{current_path_file}/num_data/{participant_name}/{participant_name}_straight.bioMod"
translate_model(q=q_0, biomod_filepath=biomod_filepath_0)


# Create the model with the knee bent
knee_dof_index = 0 # TODO change the right DoF
q_90 = np.zeros((nb_q,))
q_90[knee_dof_index] = np.pi/2
biomod_filepath_90 = f"{current_path_file}/num_data/{participant_name}/{participant_name}_bent.bioMod"
translate_model(q=q_90, biomod_filepath=biomod_filepath_90)